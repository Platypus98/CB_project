...
function checkCookie(times) {
	...
	var prompts = ["Вот так это будет выглядеть глубокой ночью:",
				   "Вот так незадолго до утренних сумерек:",
				   "Утренние сумерки:",
				   "На рассвете:",
				   "Днём:",
				   "На закате:"];
	...
	if (demo_weather == "") {
		alert("Привет Саня или Илья! Если ты это видишь, то вот так я умею подключать cookies!\n\nА теперь небольшая демонстрация, чтобы ты понял, что картинка на фоне - сложный алгоритм, а не просто говно на палке!");
		alert("Каждый день система вычисляет новые промежутки времени, в которые на фоне должен быть тот или иной пейзаж.");
		function display(j,p,v) {
			if (j == v.length) {
				alert("И так далее. Можете заходить в течении дня, наблюдать довольно прикольно");
				setCookie("visited", "true", 30);
				setTimeout(function(){location.reload()},1000);
				return
			} else {
				setPicture(true,v[j]-300000);
				setTimeout(function(){alert(p[j]);display(j+1,p,v);}, 500);
			}
		}
