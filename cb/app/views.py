...
def edit_korp_kontrol(request, id):
	...
	form1 = Data111_second_korp(request.POST or None, instance=obj)
...
def selections(request):
	...
	columns_to_display = ['proverky_gosa_po_zaprosu','proverky_gosa_po_raskritiyu','naimenovanie', 'inn', 'ogrn','kpp','data_registracii','opf','cod_emitenta','ustavnoy_capital', 
...
def home(request):
	...
	#НЕРЕАЛЬНАЯ ИСТОРИЯ-----____-___-----_____---__--__--__-__-__-__-_-__-_------___--____---____--______-______--___
	...
	users = ['В.В.Путин','Александр Калашников','Игорь Сенгаев','Олег','Ирина','','']
	types = ['0','1','Авторизация пользователя','3','4','5','6','7','8','9','10','Редактирование эмитента','12','13','14','15','16','17','18','Обновление базы',]
	icons = ['0','1','auth','3','4','5','6','7','8','9','10','edit','11','12','13','14','15','16','17','18','update']

	#if request.is_ajax():
	cursor = connection.cursor()
	cursor.execute("SELECT * FROM easyaudit_crudevent")
	...
	for i in range (3):
		dct['type'] = types[history[i]['content_type_id']]
		dct['id'] = history[i]['object_id']
		dct['date'] = history[i]['datetime']
		dct['user'] = users[history[i]['user_id']]
		dct['icon'] = icons[history[i]['content_type_id']]
		story[i] = dct


	for i in range (3):
		if story[i]['type'] == 'Редактирование эмитента':
			story[i]['type'] += ' № {}'. format(story[i]['id']) 
	...
	###---___---___----___----___---____---_____--_____--____---____--_____--_____--____--_____--____---__-
...
def update(dick,k):
	...
	'ПАО_В_СИЛУ_НАЗВАНИЯ_БЕЗ_СТ_30','НАО_СО_СТ_30','НАО_ОСУЩЕСТВИВШЕЕ_ОСУЩЕСТВЛЯЮЩЕЕ_ПУБЛИЧНОЕ_РАЗМЕЩЕНИЕ_ОБЛИГАЦИЙ_ИЛИ_ИНЫХ_ЦЕННЫХ_БУМАГ','НАО_БЕЗ_СТ_30',
...
def list(request):
	def russkie_dates(dick):
		...
		k = 0
		i = 0
		datelist = ['ДАТА РЕГИСТРАЦИИ','ДАТА ПОСЛЕДНЕЙ ОПЕРАЦИИ','ДАТА ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ',
		'ДАТА ЗАПРОСА ПО РЕЕСТРУ','ДАТА ПРЕДПИСАНИЯ ПО РЕЕСТРУ','ДАТА ПРОВЕДЕНИЯ ГОСА','ДАТА ЗАПРОСА ПО ГОСА','ДАТА ПРОВЕРКИ ГОСА',
		'ДАТА ПРЕДПИСАНИЯ ПО 1 ВЫПУСКУ','ДАТА ОПРЕДЕЛЕНИЯ СТАТУСА','ДАТА РЕШЕНИЯ ОБ ОСВОБОЖДЕНИИ','ДАТА ОТКАЗА В ОСВОБОЖДЕНИИ ОТ РАСКРЫТИЯ',
		'ДАТА ПРОВЕРКИ','ДАТА ЗАПРОСА ПО НЕ РАСКРЫТИЮ ИНФОРМАЦИИ','ДАТА ПРЕДПИСАНИЯ ПО НЕ РАСКРЫТИЮ ИНФОРМАЦИИ',
		'ДАТА ЗАПРОСА О РЕЗУЛЬТАТАХ ПРОВЕДЕНИЯ ТОРГОВ В ОБЩЕСТВЕ','ДАТА ОТВЕТА НА ЗАПРОСА О РЕЗУЛЬТАТАХ ПРОВЕДЕНИЯ ТОРГОВ В ОБЩЕСТВЕ',
		'ДАТА ЗАКЛЮЧЕНИЯ ЮРИСТАМ','ДАТА ПРОТОКОЛА','ДАТА ПОСТАНОВЛЕНИЯ','ДАТА ПИСЬМА В ФНС',
		...
		for k in range(len(datelist)):
			for i in dick['{}'.format(datelist[k])].keys():
				...
				if '00:00:00' in dick['{}'.format(datelist[k])][i]:
					dick['{}'.format(datelist[k])][i] = f'{day}.{month}.{year}'
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			sh1 = xls.parse(xls.sheet_names[0], converters={'ВИД ДЕЯТЕЛЬНОСТИ':str,'ДАТА ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ ГОСА':str,
				'НОМЕР ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ ГОСА':str,'ДАТА ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ РАСКРЫТИЕ':str,
				'НОМЕР ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ РАСКРЫТИЕ':str,'НАИМЕНОВАНИЕ':str,'ИНН':str,'ОГРН':str,'КПП':str,'ДАТА РЕГИСТРАЦИИ':str,
				'ОПФ':str,'КОД ЭМИТEHТA':str,'УСТАВНЫЙ КАПИТАЛ':str,'Количество лицевых счетов в реестре':str,'Количество номинальных держателей в реестре':str,
				'Сведения об открытии счета номинального держателя центрального депозитария':str,'РЕГИОН':str,'АДРЕС':str,'ЕДИНОЛИЧНЫЙ ИСПОЛНИТЕЛЬНЫЙ ОРГАН':str,
				'КОНТАКТНЫЕ ДАННЫЕ':str,'СТАТУС':str,'ДВИЖЕНИЕ ДЕНЕЖНЫХ СРЕДСТВ':str,'ДАТА ПОСЛЕДНЕЙ ОПЕРАЦИИ':str,'ОТЧЕТНОСТЬ':str,'ЗАДОЛЖЕННОСТЬ ПЕРЕД ФНС':str,
				'КАРТОЧКА КОМПАНИИ':str,'РЕГИСТРАТОР':str,'НАИМЕНОВАНИЕ РЕГИСТРАТОРА':str,'ДАТА ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ':str,
				'НОМЕР ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ':str,'ДАТА ЗАПРОСА ПО РЕЕСТРУ':str,'НОМЕР ЗАПРОСА ПО РЕЕСТРУ':str,'ДАТА ПРЕДПИСАНИЯ ПО РЕЕСТРУ':str,
				'НОМЕР ПРЕДПИСАНИЯ ПО РЕЕСТРУ':str,'ПРОВЕРКИ ГОСА ПО РАСКРЫТИЮ':str,'ПРОВЕРКИ ГОСА ПО ЗАПРОСУ':str,'ДАТА ПРОВЕДЕНИЯ ГОСА':str,'ДАТА ЗАПРОСА ПО ГОСА':str,
				'НОМЕР ЗАПРОСА ПО ГОСА':str,'ДАТА ПРОВЕРКИ ГОСА':str,'ПРОВЕРКА 1 ВЫПУСК':str,'ДАТА ПРЕДПИСАНИЯ ПО 1 ВЫПУСКУ':str,'НОМЕР ПРЕДПИСАНИЯ ПО 1 ВЫПУСКУ':str,
				'НРД':str,'ПРОВЕРКИ НРД':str,'ОАО на 22.06.2015':str,'КОРП КОНТРОЛЬ':str,'ПАО В СИЛУ ПРИЗНАКОВ СТ.30':str,'ПАО В СИЛУ ПРИЗНАКОВ БЕЗ СТ. 30':str,
				'ПАО В СИЛУ НАЗВАНИЯ СТ. 30':str,'ПАО В СИЛУ НАЗВАНИЯ БЕЗ СТ. 30':str,'НАО СО СТ. 30':str,
				'НАО ОСУЩЕСТВИВШЕЕ (ОСУЩЕСТВЛЯЮЩЕЕ) ПУБЛИЧНОЕ РАЗМЕЩЕНИЕ ОБЛИГАЦИЙ ИЛИ ИНЫХ ЦЕННЫХ БУМАГ':str,'НАО БЕЗ СТ. 30':str,
				'ДАТА ОПРЕДЕЛЕНИЯ СТАТУСА':str,'ОТКАЗ В РЕГИСТРАЦИИ ВЫПУСКА':str,'ОСВОБОЖДЕНЫ ОТ РАСКРЫТИЯ':str,'ДАТА РЕШЕНИЯ ОБ ОСВОБОЖДЕНИИ':str,
				'НОМЕР РЕШЕНИЯ ОБ ОСВОБОЖДЕНИИ':str,'ОТКАЗ В ОСВОБОЖДЕНИИ ОТ РАСКРЫТИЯ':str,'ДАТА ОТКАЗА В ОСВОБОЖДЕНИИ ОТ РАСКРЫТИЯ':str,
				'НОМЕР ОТКАЗА В ОСВОБОЖДЕНИИ ОТ РАСКРЫТИЯ':str,'ПРОВЕРКА РАСКРЫТИЯ':str,'ДАТА ПРОВЕРКИ':str,'ДАТА ЗАПРОСА ПО НЕ РАСКРЫТИЮ ИНФОРМАЦИИ':str,
				'НОМЕР ЗАПРОСА ПО НЕ РАСКРЫТИЮ ИНФОРМАЦИИ':str,'ДАТА ПРЕДПИСАНИЯ ПО НЕ РАСКРЫТИЮ ИНФОРМАЦИИ':str,'НОМЕР ПРЕДПИСАНИЯ ПО НЕ РАСКРЫТИЮ ИНФОРМАЦИИ':str,
				'ДАТА ЗАПРОСА О РЕЗУЛЬТАТАХ ПРОВЕДЕНИЯ ТОРГОВ В ОБЩЕСТВЕ':str,'НОМЕР ЗАПРОСА О РЕЗУЛЬТАТАХ ПРОВЕДЕНИЯ ТОРГОВ В ОБЩЕСТВЕ':str,
				'ДАТА ОТВЕТА НА ЗАПРОСА О РЕЗУЛЬТАТАХ ПРОВЕДЕНИЯ ТОРГОВ В ОБЩЕСТВЕ':str,'НОМЕР ОТВЕТА НА ЗАПРОСА О РЕЗУЛЬТАТАХ ПРОВЕДЕНИЯ ТОРГОВ В ОБЩЕСТВЕ':str,
				'ВЫВОД':str,'РАСКРЫТИЕ':str,'ДАТА ЗАКЛЮЧЕНИЯ ЮРИСТАМ':str,'НОМЕР ЗАКЛЮЧЕНИЯ ЮРИСТАМ':str,'ДАТА ПРОТОКОЛА':str,'НОМЕР ПРОТОКОЛА':str,'СТАТЬЯ КОАП':str,
				'ДАТА ПОСТАНОВЛЕНИЯ':str,'НОМЕР ПОСТАНОВЛЕНИЯ':str,'РЕЗУЛЬТАТ':str,'РАЗМЕР ШТРАФА':str,'АДМИНИСТРАТИВКА':str,'ФНС':str,'ДАТА ПИСЬМА В ФНС':str,'НОМЕР ПИСЬМА В ФНС':str,
				'ИНФОРМАЦИЯ О ПОЛУЧЕНИИ ОТВЕТА ОТ ФНС':str,'ВХ. НОМЕР ОТВЕТА':str,'СОДЕРЖАНИЕ ОТВЕТА':str,'ОТВЕТ ФНС ОБ АДРЕСЕ':str,'СВЕДЕНИЯ ОБ АДРЕСЕ НЕ ДОСТОВЕРНЫ':str,
				'ВЗАИМОДЕЙСТВИЕ С ФНС НА ЕЖЕКВАРТАЛЬНОЙ ОСНОВЕ':str,'ВЗАИМОДЕЙСТВИЕ С ФНС НА ЕЖЕКВАРТАЛЬНОЙ ОСНОВЕ (НОМЕР ПИСЬМА)':str,'ВЗАИМОДЕЙСТВИЕ С ФНС НА ЕЖЕКВАРТАЛЬНОЙ ОСНОВЕ (ДАТА ПИСЬМА)':str,
				'ДАТА ВНЕСЕНИЯ ЗАПИСИ':str,'ВЗАИМОДЕЙСТВИЕ С ГОС. ОРГАНАМИ':str })
			sh1 = sh1.fillna('')
			...
			#-------------------------Сохранение в буфер столцов, которых нет в эксель
			...
			cache.set('main_dict', excel_dict1)
	...
	if request.method == 'GET' and request.is_ajax():
		...
		if k == 95:
			...
			#cursor.execute("SELECT MAX(datetime) FROM easyaudit_crudevent WHERE (content_type_id = 19)")
			cursor.execute("COMMIT;")
			cursor.close()
